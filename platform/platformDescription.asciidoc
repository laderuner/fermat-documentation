...
:leveloffset: 2
include::subapps/brandAndRetailer.asciidoc[]

:leveloffset: 2
include::subapps/cryptoBroker.asciidoc[]

:leveloffset: 2
include::subapps/topUp-CashOutPoint.asciidoc[]
...
Each of this subsystem is divided in layers. Inside the layers we can find plug-ins, each plug-in has a specific task to do. As a convention we call the plug-ins inside Fermat-PIP Addons instead of Plug-ins. +
...
We can also identify outside this subsystems two main structures. One is whar we call fermat-pip and the other is fermat-core.
