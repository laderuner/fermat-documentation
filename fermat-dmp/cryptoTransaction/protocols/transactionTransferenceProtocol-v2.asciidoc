...
* Por cuestiones de solventar problemas de concurrencia, optamos por usar métodos sincronizados (usamos el modificador synchronized) en la interfaz `TransactionSender`. Además notemos que podemos parametrizar las Transacciones para que transporten otro tipo de datos fuera de transacciones crypto, esto será de utilidad para reutilizar el código. La interfaz será entonces:

  public interface TransactionSender<E> {
    public synchronized void confirmReception(UUID transactionId)
    public synchronized List<Transaction<E>> getPendingTransactions(Specialist specialist)
  }

* El Incoming crypto usará una clase (Registry) que compartirán los dos agentes de cada módulo. Manejará los siguientes métodos (*notar el modificador synchronized*):

NOTE: EventWrapper es un contenedor de datos de información de eventos. Probablemente no hace falta.

  //To manage events
  synchronized void saveNewEvent(String eventType, String eventSource) throws CantSaveEvent;
  synchronized EventWrapper getNextPendingEvent() throws CantReadEvent;
  synchronized void discardEvent(UUID eventId) throws CantReadEvent, CantSaveEvent;
  
  //To manage transactions
  
  //Monitor Agent
  // Las coloca en (A,TBN)
  synchronized void acknowledgeTransactions(List<Transaction<CryptoTransaction>> transactionList) CantAcknowledgeTransactionException;
  // Retorna las que están en (A,TBN)
  synchronized List<Transaction<CryptoTransaction>> getAcknowledgedTransactions() CantGetTransactionsException;
  // Pasa una de (R,NAR) 
  synchronized void acquireResponsibility(Transaction<CryptoTransaction> transaction) CantAcquireResponsibility;
  
  //Relay Agent
  // Retorna las (R,NAR)
  synchronized List<Transaction<CryptoTransaction>> getResponsibleTransactions() CantAccessTransactionsException;
  // Pasa la transacción a TBN y le agrega el Specialist.
  synchronized void setToNotify(UUID id, Specialist specialist)
  // La lista de (R,TBN) o (R,SN)
  synchronized List<Transaction<CryptoTransaction>> getResponsibleTransactionsPendingAction() CantAccessTransactionsException;
  // Da los Specialist de las que están en TBN y SN
  synchronized List<Specialist> getSpecialists() CantReadSpecialistsException;
  // Pasa las que son TBN a SN
  synchronized void setSendingNotified() CantSaveTransactionsException;

El registry mismo implementará la interfaz TransactionSender<CryptoTransaction>

== Analizar

¿Se pueden fusionar el Monitor y el RelayAgent en un sólo thread?

LUIS: Yo prefiero que no. Dejaría una tarea a cada quien y se verían mas simples ambos. Pero en teoría, poder se puede.
