...
```
  ...
  El método getPendingTransactions retorna la lista de todas las transacciones con ProtocolStatus SENDING_NOTIFIED que tienen como Specialist al pasado como argumento. *No camba el estado de ninguna transacción*
...
. Un agente del IncomingCryptoExtraUser ve que hay transacciones pendientes y llama al método `getPendingTransactions(EXTRA_USER)`, luego recorre la lista que devuelve y *primero guarda en su base de datos* a todas las transacciones asignándoles el estado (ACKNOWLEDGE,TO_BE_NOTIFIED). *Luego*, por cada transacción en la base de datos con estad (ACKNOWLEDGE,TO_BE_NOTIFIED) (que pueden ser más que las que guardó recién) ejecuta los siguientes pasos *en ese orden*:
...
.. Marca la transacción como (RESPONSIBLE,NO_ACTION_REQUIRED)
. El segundo agente ve las transacciones que tiene y decide si debe ejecutarlas o no, *luego de ejecutar la transacción* la marca como (APPLIED,NO_ACTION_REQUIRED)
