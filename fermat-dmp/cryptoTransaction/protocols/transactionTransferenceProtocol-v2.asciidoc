...
Lo que implementé fue el Registry así que dejo a su elección la implementación que considere mejor (si agregar lo del transaction manager o dejar simplemente la intefaz transaction sender). Como dije, yo prefiero la segunda opción.
...
* Analizar si pude quitarse el parámetro Specialist.
...
La variable databaseTable es un miembro de la clase, por lo que sería compartidas por distintos threads threads. +
Eso a mi entender trae problemas porque si dos threads llaman a métodos que requieren tablas distintas de la base de datos tendría trazas que hacen que el pedido de un thread pise al del otro. +
