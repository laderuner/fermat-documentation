...
== Transaction Transfer Protocol (DRAFT)
...
* <<Fermat-DMP-CryptoTransactionLayer-TransactionTransferProtocol-EBDProtocol,Más adelante>> hablaremos del caso de una comunicación que involucra tres módulos, uno de tipo T (emisor original), uno de tipo R (receptor final) y un módulo que actúa como tipo R para el emisor y como tipo T para el receptor final (un puente).
...
[[Fermat-DMP-CryptoTransactionLayer-TransactionTransferProtocol-EBDProtocol]]
=== Caso de tres módulos

Llamaremos a este caso el Protocolo EBD (Protocolo **E**mitter-**B**ridge-**D**estination). +
...
La variación es leve, Los módulos E y D actuarán como explicamos antes en el protocolo simple (no saben que hay un puente), el puente "simplemente" agregará un cuarto thread que lea la tabla que tiene el thread 3 y actúa como un módulo tipo T pero toma las transacciones en estado NOTIFIED en lugar de NEW. +
Al esperar que estén en NOTIFIED en lugar de NEW se garantiza que el traspazo de la transacción de E a B fue completado antes de comenzar a transmitir de B a D.
