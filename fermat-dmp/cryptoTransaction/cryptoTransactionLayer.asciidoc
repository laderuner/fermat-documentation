...
Para tener esto en cuanta en los pasos del protocolo, a cada transacción se le agregará un estado de la transacción y un estado de notificación, que notaremos (ESTADO_DE_TRANSACCIÓN, ESTADO_DE_NOTIFICACIÓN). +
...
El significado y use de cada estado se explicará a continuación junto con el protocolo.
...
. T controla si tiene transacciones en la combinación (RESPONSIBLE,TO_BE_NOTIFIED). 
** Si tiene transacciones en ese estado, envía el evento TransactionsWaitingTransferenceEvent, para que un módulo responsable se haga cargo y *luego de envíar el evento* pasa las transacciones del estado (RESPONSIBLE,TO_BE_NOTIFIED) al estado (RESPONSIBLE, NOTIFIED).
** Si no tiene transacciones en ese estado sigue con el análisis del resto de las transacciones.
. R escucha el evento que lanzó T, mira que el origen del evento es T y llama a un método de la interfaz de T que retorna una lista con todas las transacciones que tiene guardadas en estado (RESPONSIBLE, NOTIFIED). **T no cambia el estado de las transacciones que tiene almacenadas cuando R hace esta consulta.**
...
. Luego, para cada transacción en estado (ACKNOWLEDGED, TO_BE_NOTIFIED) en su base de datos, R llama a otro método de T al cual le pasa el ID de la transacción. La ejecución de este método marca en la base de datos de T a dicha transacción como (DELIVERED, NO_ACTION_REQUIRED). *Luego de esa llamada al método de T*, R marca la transacción en estado (RESPONSIBLE, NOTIFIED). El motivo por el cual primero se notifica y luego se confirma se hace para garantizar que la transacción fue correctamente confirmada por T antes de marcar que lo fue en la base de datos de R.
...
[start=5]
. R toma las transacciones en estado (RESPONSIBLE, NOTIFIED) y realiza su tarea. *Luego de aplcar la tarea* las pasa a (RESPONSIBLE, APPLIED)
...
NOTE: La separación entre el agente 2 y el agente 3 es necesaria porque si se notoficara que llegó una transacción antes de marcarla como NEW y luego se callera el sistema, la transacción no se habría registrado y el emisor la tendría marcada como notificada.
...
NOTA PARA EZEQUIEL: Hasta acá todo bien en general. Lo unico que yo ajustaría sería la nomenclatura para que se alinie aun mas con la forma en que lo explicamos oralmente. Y ahí vuelvo al concepto de la responsabilidad de cada módulo. 
