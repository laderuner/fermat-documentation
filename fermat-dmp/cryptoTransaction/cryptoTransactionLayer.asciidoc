...
The value at this moment is stored in the crypto currencies assign to the user addresses managed by the wallets. On the other hand the information tell us, among other things, in which wallet is allocated value, what that value represents and what amount is allocated in each wallet. +
