...
Aquí el paso 6 será que el tercer agente de B decide a quién debe enviar algunas transacciones, las pasa al estado (RESPONSIBLE, TO_BE_NOTIFIED)  (combinación de estados que no se da en el protocolo simple) y marca de alguna manera el evento a lanzar según el análisis realizado. 
...
La implementación del método `getPendingTransactions` va a requerir que tome un parámetro que refleje de cierta forma qué transacciones se deben entregar. Recordemos que un punte puede tener varios receptores finales y por ende debe clasificar qué transacciones debe dar a cada receptor. +

IMPORTANT: Probablemente una buena idea sea crear un enum que represente una metadata de la transacción. Esto permitiría no sólo distinguir a quién mandamos la transacción sino también otro tipo de datos, por ejemplo el crypto estado de las transacciones. Así cuando haya un reversed se podría pedir la transacción poniendo como metadata un flag que indique que se entreguen las reversed que no se han notificado. +
Otra opción sería tener dos parámetros, el metadata por un lado y el de "id de destino" por otro lado. 
...
Estos protocolos serán implementado por diversos plug-ins que interactúan con esta capa.
...
* Uno semántico es que temporalmente ambos módulos (R y T) tendrán la misma transacción con el estado RESPONSABLE, en cuyo caso podríamos asumir que ambos son responsables de esa transacción (como dijimos antes, esto no tiene problemas de implementación).
...
A mí ambos me parecen entendibles, pero yo soy el que lo está haciendo y lo que me interesa es que sea entendible a otros que puedan llegar a tener que tocar el código. +
Por otro lado tengo más plug-ins qué pensar así que tampoco quiero seguir dándole tanto tiempo a este tema si es que tenemos prioridad por hacer algo rápido. Voy a dejar la versión que explico aqui y paso a lo demás. Si vemos que hay tiempo lo mejoramos. +
...
Así que para notificar traspasos tal vez podmos crear eventos

* TransactionsWaitingTransferenceIntraUserEvent
* TransactionsWaitingTransferenceExtraUserEvent
* Habría que tener uno para DeviceUser? (A FUTURO)
