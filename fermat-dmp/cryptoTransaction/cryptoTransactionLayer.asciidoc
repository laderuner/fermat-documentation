...
Aquí el paso 7 será que el tercer agente de B decide a quién debe enviar algunas transacciones, las pasa al estado (RESPONSIBLE, TO_BE_NOTIFIED)  (combinación de estados que no se da en el protocolo simple) y marca de alguna manera el evento a lanzar según el análisis realizado. 
...
A mí ambos planteos (el descripto y uno tras optimizar un poco fusionando threads) me parece que serían entendibles, pero yo soy el que lo está haciendo y lo que me interesa es que sea entendible a otros que puedan llegar a tener que tocar el código. +
