...
** Si tiene transacciones en estado (RESPONSIBLE, TO_BE_NOTIFIED) o (RESPONSIBLE, NOTIFIED), envía el evento TransactionsWaitingTransferenceEvent, para que un módulo responsable se haga cargo y *luego de envíar el evento* pasa las transacciones del estado (RESPONSIBLE,TO_BE_NOTIFIED) al estado (RESPONSIBLE, NOTIFIED). 
...
.NOTAS DEL PROTOCOLO
* Cuando se involucra sólo a dos módulos (uno de tipo R y otro T), este protocolo se implementa con 3 agentes en R y dos en T.
...
** El segundo agente mira la tabla que llena el agente anterior con transacciones nuevas y le informa a T que recibió correctamente las transacciones de a una a la vez. Entonces, *tras confirmarle a T la recepción* pasa la misma al estado (RESPONSIBLE, NOTIFIED).
...
** El tercer agente es quien se despierta periódicamente, lee las transacciones en estado (RESPONSIBLE, TO_BE_NOTIFIED) o (RESPONSIBLE, NOTIFIED) y envía el evento TransactionsWaitingTransferenceEvent y marca las que eran TO_BE_NOTIFIED como (RESPONSIBLE, NOTIFIED). *Notar que sigue en estado RESPONSIBLE*.

NOTE: La separación entre el agente 1 y el agente 2 de R es necesaria porque si se notoficara que llegó una transacción antes de marcarla como (ACKNOWLEDGED, TO_BE_NOTIFIED) y luego se callera el sistema, la transacción no se habría registrado y el emisor la tendría marcada como notificada.
